...
// Default passcode
#define DEFAULT_PASSCODE						123456 //0//19655
...
static uint8 scanRspData[] =
{
	...
	0xFF, 0xFF, 
	0x11, 0x22, 0x33, 0x44, 0x55, 0x00, // mac add
	0x66,
    0xFF,
    0x03, 0x04,
	0x00,
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "GATT--DEMOS";
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
static GUA_PAIRSTATUS gGUA_PairStatus = GUA_PAIRSTATUS_NO_PAIRED;  
...
{  
 ...
 // uint8 str[7];  
  ...
  //  passcode %= 1000000;  
  //#else  
    passcode = DEFAULT_PASSCODE; //456890;          
  //#endif  
 LOG("Passwd proses = %d \n",passcode);  
...
{
  //GUA_ProcessPasscodeCB,                        //????  
  //GUA_ProcessPairStateCB                        //??????  
 ...
 */
extern BUF bfr;
extern uint8 rx_idx,idx;
//extern struct jadwal_sholat jd;
uint16 id=1;
uint16 file_len;
int ret;
...
//#if (DEF_GAPBOND_MGR_ENABLE==1)    
    ...
    //{
		//uint32 passkey = 0; //DEFAULT_PASSCODE;
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
   ...
   // #endif
		...
		//ret = hal_fs_item_read(id,(uint8*)bfr.bfr.dt_rx,10,0);
		//dbg_printf("fg=%d-R=%s  len=%d\n",ret,bfr.bfr.dt_rx,&file_len);
		//bfr.bfr.dt_rx[0]=1; bfr.bfr.dt_rx[1]=2 ; bfr.bfr.dt_rx[2]='A'; bfr.bfr.dt_rx[3]='D';
		//LOG("Write=%d\n",hal_fs_item_write(fs_bios,(uint8*)bfr.bfr.dt_rx,4));
