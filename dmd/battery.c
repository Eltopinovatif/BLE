...
static void batt_adc_evt(adc_Evt_t* pev)
{
  ...
  if(pev->type == HAL_ADC_EVT_DATA){
    p_ctx->vbat_value = get_batt_value(pev->data, pev->size);
	  LOG("batteray:%d",p_ctx->vbat_value);
  }
  //LOG("batt:%d\n",p_ctx->vbat_value);
}
...
{
	 s_batt_ctx.vbat_value = s_batt_ctx.vbat_value;
...
static int batt_adc_init(void)
{
	int ret;
  adc_Cfg_t cfg = {
    .channel = ADC_BIT(ADC_CH3P_P20), //ADC_BIT(ADC_CH3P_P20)|ADC_BIT(ADC_CH2P_P14)|ADC_BIT(ADC_CH3N_P15),
